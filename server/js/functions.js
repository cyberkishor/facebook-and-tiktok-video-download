...
(function ($) {
  ...
  function togglePlay (video, circlePlayButton) {
    if (video.get(0).paused || video.get(0).ended) {
      video.get(0).play();
      circlePlayButton.css("opacity", 0);
      circlePlayButton.addClass('playing');
    } else {
      video.get(0).pause();
      circlePlayButton.css("opacity", 1);
      circlePlayButton.removeClass('playing');
    }
  }


  jQuery('.play-gif').on('click', function () {
    var video = jQuery(this).parents('.project-item').find('video');
    var circlePlayButton = jQuery(this);
    video = jQuery(video)
    togglePlay(video, circlePlayButton);
  });
